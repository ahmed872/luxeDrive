...
describe('recordAuditEvent', () => {
  it('records a login success against the user', async () => {
    ...
    expect(rows[0]).toMatchObject({ action: 'auth.login.success', entityType: 'User', ip: '1.2.3.4' });
  });

  it('records a failed login with no user row (unknown email) without a foreign key', async () => {
    await recordAuditEvent({ action: 'auth.login.failure', userId: null, before: { reason: 'NOT_FOUND' } });
  ...
  it('records a role change with before/after snapshots', async () => {
    const user = await createUser({ email: 'owner@example.com', password: 'correct-horse-9', role: 'STAFF' });
